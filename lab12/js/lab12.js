...
var submitButton = document.getElementById("button");//get the button in the HTML
submitButton.addEventListener('click', function(){//when the buttton is clicked
  var name = document.getElementById("input").value;//get the value of the name enetered in the input field and apply it to sortingHat function.
  var house = sortingHat(name);//Your house is...
...
//   document.getElementById("problem").innerHTML = "<p>This lab was particularly arduous as this time I had many problems. Utilizing the methods from lab 10 I was able to create buttons that functioned identically to the ones present in this lab, once I had translated my previous code to work, I then had to convert it to a JQuery format as well ass assigning both a class and unique div to each one. This lead to multiple iterations of essentially the same code and experimenting with what could be replaced by '$' and what could not, in some cases it would read correctly, in others it would read the wrong things or not read at all. Implementing a button that would also toggle the CSS attributes was also very difficult as a review of the lecture was of no help in that area. Initially, it was taken from w3 schools and implemented directly into the HTML file via a script, however this applied its attributes to only the example instead of the rest of the page or specific tags and classes that were given. It took some time but through much trial and error, I managed to both reduce the bulk of the code and remove it from the HTML where it was no longer needed.</p>"
// });
//
